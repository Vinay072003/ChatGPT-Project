...
void CloudSpeechClient::Transcribe(Audio* audio) {
  String HttpBody1 = "{\"config\":{\"encoding\":\"LINEAR16\",\"sampleRateHertz\":16000,\"languageCode\":\"en-IN\",\"enableWordTimeOffsets\":1},\"audio\":{\"content\":\"";
  String HttpBody3 = "\"}}\r\n\r\n";
  ...
  while (client.available())
  {
    ...
    My_Answer = My_Answer + client.readString();
    // Serial.write(client.read());
    // delay(100);
  }

   Serial.print("My Answer - ");
  //  Serial2.println(My_Answer);
...
// Serial.print(ans);
  ...
  //  client.setInsecure();
    ...
    if (https.begin(client,"https://api.openai.com/v1/chat/completions")) {  // HTTPS
      ...
      //  String payload = String("{\"model\": \"" + OpenAI_Model + "\", \"prompt\": ") + "\"" + chatgpt_Q + "\"" + String(", \"temperature\": " + OpenAI_Temperature + ", \"max_tokens\": " + OpenAI_Max_Tokens + "}"); //Instead of TEXT as Payload, can be JSON as Paylaod
      String payload = String("{\"model\": \"" + OpenAI_Model + "\", \"messages\": [{\"role\": \"user\", \"content\": \"" + chatgpt_Q + "\"}], \"temperature\": " + OpenAI_Temperature + ", \"max_tokens\": " + OpenAI_Max_Tokens + "}");
    ...
    Serial.print(payload);
      ...
      if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_MOVED_PERMANENTLY) {
        ...
        const char* only_ans = choices_0["message"]["content"];
